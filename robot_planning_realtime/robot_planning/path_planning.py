...
class CartesianPathPlanner:
    ...
    def plan(self, start: Point3D, goal: Point3D) -> Optional[Path3D]:
        ...
        for i in range(self.max_iterations):
            ...
            # Progress reporting every 200 iterations instead of every iteration
            if i > 0 and i % 200 == 0:
                logger.debug(f"Planning iteration {i}/{self.max_iterations}, trees: {len(self.tree_start['points'])}/{len(self.tree_goal['points'])}")
            ...
            if new_idx is not None:
                ...
                if self._try_connect(tree_from, tree_to, new_idx):
                    ...
                    if self.best_path and len(self.best_path) < 50:  # Stop if path is reasonable length
                        logger.debug(f"Early exit with reasonable path length: {len(self.best_path)}")
                        break
        ...
        if self.best_path:
            ...
            simplified_path = self._simplify_path(self.best_path)
            logger.info(f"Path simplified from {len(self.best_path)} to {len(simplified_path)} waypoints")
            # Smooth the simplified path
            smoothed_path = self.smooth_path(simplified_path, iterations=50)
    ...
    def _simplify_path(self, path: Path3D, max_waypoints: int = 30) -> Path3D:
        """
        Simplify path by removing unnecessary waypoints while preserving collision-free property.
        
        Args:
            path: Original path to simplify
        ...
        # Use Douglas-Peucker-like algorithm to keep important waypoints
        simplified = [path[0]]  # Always keep start
        
        # Calculate target step size to get approximately max_waypoints
        step_size = len(path) // max_waypoints
        
        for i in range(step_size, len(path) - step_size, step_size):
            # Check if we can skip directly from last added point to path[i]
            if len(simplified) > 0 and self.env.is_path_valid(
                simplified[-1], path[i], 
                num_steps=max(5, int(np.linalg.norm(path[i] - simplified[-1]) / 0.02)),
                robot_controller=self.robot_controller
            ):
                # We can skip intermediate points, but add this one
                simplified.append(path[i])
            else:
                # Need to add intermediate waypoints
                if len(simplified) > 0:
                    # Add the previous point that we might have skipped
                    prev_idx = i - step_size
                    if prev_idx > 0 and not np.allclose(simplified[-1], path[prev_idx]):
                        simplified.append(path[prev_idx])
                simplified.append(path[i])
    ...
    def smooth_path(self, path: Path3D, iterations: int = 30, 
                   smoothness_weight: float = 0.5, 
                   obstacle_weight: float = 0.3,
                   learning_rate: float = 0.15) -> Path3D:
        """
